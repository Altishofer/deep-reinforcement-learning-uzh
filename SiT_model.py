...
class TinySiETPolicy(nn.Module):
    def __init__(self, in_ch, num_actions, h, w):
        super().__init__()
        assert h == w, "TinySiet expects square images"
        ...
        # Do NOT probe the backbone here. We hardcode a stable head size.
        self.feat_dim = 64  # single number to fix heads

        # Lazy adapter Linear(in_dim_detected_at_runtime -> 64)
        self.adapter = None

        # PPO heads
        self.pi = nn.Linear(self.feat_dim, num_actions)
        self.v = nn.Linear(self.feat_dim, 1)
        nn.init.orthogonal_(self.pi.weight, gain=0.01)
        nn.init.constant_(self.pi.bias, 0.0)
        nn.init.orthogonal_(self.v.weight, gain=1.0)
        nn.init.constant_(self.v.bias, 0.0)

    def _prepare_features(self, z):
        # Convert backbone output to [B, F_in]
        if z.dim() == 4:
            B = z.shape[0]
            z = z.view(B, -1)
        elif z.dim() == 3:
            # [B, T, F] -> mean pool tokens
            z = z.mean(dim=1)
        elif z.dim() == 2:
            pass
        else:
    ...
    def forward(self, x):
        # Try normal batch first
        try:
            # IMPORTANT: call backbone per-sample to avoid the internal reshape crash
            # The crash happens even for bs=1, but we will catch and handle feature mapping consistently.
            z_list = []
            for i in range(x.shape[0]):
                zi = self.backbone(x[i:i + 1])  # run one sample at a time
                z_list.append(zi)
            try:
                z = torch.cat(z_list, dim=0)
            except Exception:
                z = torch.cat([zi.view(zi.shape[0], -1) for zi in z_list], dim=0)
        except RuntimeError:
            # If anything else goes wrong, fall back to flatten
            z = x.view(x.shape[0], -1)

        # Prepare features for adapter
        z = self._prepare_features(z)
...
class PPO:
    def __init__(self, cfg: Config):
        ...
        self.last_obs = obs0
        self.global_step = 0
        self.metrics = {"update": [], "policy_loss": [], "value_loss": []}
    ...
    def update(self, obs_buf, act_buf, logp_buf, adv_buf, ret_buf):
        ...
        return np.mean(ploss_acc), np.mean(vloss_acc)
...
if __name__ == "__main__":
    ...
    agent = PPO(cfg)
    agent.train()
    torch.save(agent.net.state_dict(), f"ppo_{cfg.env_name}_siet_debug.pt")
